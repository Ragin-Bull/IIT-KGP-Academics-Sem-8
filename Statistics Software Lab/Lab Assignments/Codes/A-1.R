...
simpsons_integration <- function(func, alpha, beta, num){
  if (num %% 2 != 0) {
    stop("Number of subintervals (n) shoule be divisible by 2")
  }
  
  step_s <- (beta - alpha) / num
  
  res <- func(alpha) + func(beta)
  
  for (i in 1:(n-1)) {
    x_i <- alpha + i * step_s
    res <- res + 2 * func(x_i) * (1 + i %% 2)
  }
  res <- res * h / 3
  return(res)
}

get_W <- function(X , pdf, n){
  ...
  for(i in 1:n){
    cdf <- simpsons_integration (pdf,ints[i],ints[i+1],1000)
    E[i] <- cdf*num
    O[i] <- length(X[X<=ints[i+1]])-length(X[X<ints[i]])
  }
  
  result <- sum((O-E)^2/E)
  critical_value <- qchisq(0.95,num-1)
